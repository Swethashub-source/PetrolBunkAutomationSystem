...
/*public class Delete_DieselPrice {
...
import java.util.Date;
 ...
 public class Delete_DieselPrice extends JFrame{
	 ...
	 private static final long serialVersionUID = 1L;
		...
		public Delete_DieselPrice()
		{
			...
			try {
				rs=stmt.executeQuery("select price,city,area,day from diesel_price");
				while(rs.next()) {
			...
			table.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent evt) {
					...
					/*try {
						rs=stmt.executeQuery("select price from petrol_price where city='"+bcity.getText()+"' and area='"+barea.getText()+"' and day="+bdate.getText()+"");
						bprice.setText(rs.getString(1));
					}
					catch (SQLException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
			...
			in.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					try {
						String sql="DELETE FROM diesel_price  where city=? and area=? and day=?";
						PreparedStatement statement=con.prepareStatement(sql);
						statement.setString(1,bcity.getText());
						statement.setString(2,barea.getText());
						statement.setString(3,bdate.getText());
					...
					ta.append("Deleted "+i+" rows successfully\n");
				}
			});
