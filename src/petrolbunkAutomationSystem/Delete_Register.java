...
public class Delete_Register extends JFrame{
	...
	public Delete_Register() {
		...
		setVisible(true);
		setSize(500,600);
		setTitle("Delete Registered Account:");
		...
		lis.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent arg0) {
				...
				try {
					rs=stmt.executeQuery("select name,phno,pin from register where phno="+lis.getSelectedItem()+"");
					if(rs.next()) {
						uname.setText(rs.getString(1));
						upin.setText(rs.getString(3));
						uphno.setText(rs.getString(2));
